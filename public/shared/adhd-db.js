...
(function(){
  ...
  async function listClientsPaged(q) {
    ...
    if (q.dept) {
      // dept는 라우팅 라벨일 뿐 환자 소속 아님(환자는 분과 무관) → 목록 거르지 않음(org 전체 노출).
    }
  ...
  // ============ CONSENT (adhd_consent — client_id 귀속, org 격리 RLS) ============
  ...
  // [B-① 동의서 관리] org 전체 동의서 목록 (인증된 임상가=RLS org격리). super(비인증)는 edge(service_role) 사용.
  //   회원 본인(is_self=true) + 환자(is_self=false). 서명이미지·만료 포함(법적 근거 열람).
  async function listConsent() {
    const c = getClient();
    const { data, error } = await c.from('adhd_consent')
      .select('id,client_id,consent_type,signer_name,sign_method,consent_version,signed_at,expires_at,signature,scope,data_types, adhd_clients(is_self,self_user_id,full_name)')
      .eq('organization_id', orgId())
  ...
  window.ADHD_DB = {
    ...
    // consent
    saveConsent, getConsent, listConsent,
    // assessments
