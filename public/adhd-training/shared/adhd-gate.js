...
<h3>14. 문의</h3><p>문의: bostonneuromind@gmail.com · Boston Neuromind LLC, 매사추세츠 Canton.</p>`;
  ...
  function viewConsent(ctx) {
    ...
    card.innerHTML = `
      ${headerHtml(t('연구·교육·임상 동의서', 'Research, Education & Clinical Consent'))}
    ...
    if (sendBtn) sendBtn.onclick = async () => {
      ...
      try {
        const res = await fetch(((window.ADHD_CONFIG && ADHD_CONFIG.apiBase) || '') + '/api/v1/adhd/consent_send', {
          method: 'POST',
  ...
  function viewConsentToken(token) {
    ...
    $('#ctBtn', card).onclick = async () => {
      ...
      try {
        const res = await fetch(((window.ADHD_CONFIG && ADHD_CONFIG.apiBase) || '') + '/api/v1/adhd/consent_sign', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            token: token,
  ...
  async function require() {
    ...
    const isConsentExempt = session.role === 'admin' || session.role === 'super';   // ★ 동의: owner(임상가)도 받게, admin/super만 면제 (cid 강제 isAdmin과 분리)
    // ★ 동의 게이트 — consumer 본인만. self_client_id가 client_code일 수 있어 실제
    //   adhd_clients.id(uuid)로 해석(consent_send/saveConsent가 adhd_consent.client_id에
    //   uuid를 넣어야 함 — 비-uuid면 Postgres uuid 캐스팅 500). 현장·이메일 양쪽 동시 해결.
