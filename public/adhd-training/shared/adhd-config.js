...
window.ADHD_CONFIG = {
  // ★ DB 통합: sepiera (할비 결정 2026-06 — peak 환자관리를 talk방식 clinician+get-link로).
  ...
  symptom:    'peak',           // peak 분과 정체성(데이터 태그·관문 dept 매칭)
  tablePrefix:'adhd_',          // 11봇 복사 시 prefix rename과 일치
  hue:        '#2D9B87',        // Peak=Teal (attention 인디고와 구분)

  // 판매 모델: 'consumer'(셀프호스트 단일org) | 'professional'(SaaS 클리닉 테넌트)
  deployMode: 'consumer',
  siteKey:    'peak',           // [source-site] 가입 RPC p_source_site로 전달 (사이트 라벨)
  ...
  // 환자 매직링크 착지(origin 기준) — peak 환자는 peak 홈으로.
