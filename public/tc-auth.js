...
(function () {
  ...
  async function signUp({ name, email, password, lang, role, birthdate, isMinor }) {
    const c = client(); if (!c) return NOT_CONFIGURED;
    return await c.auth.signUp({
      email: email, password: password,
      options: {
        data: {
          display_name: name || '', language: lang || 'en', role: role || 'user',
          birthdate: birthdate || '', is_minor: !!isMinor,
          parent_consent_status: isMinor ? 'pending' : '',
        },
